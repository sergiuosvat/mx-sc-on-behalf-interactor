...
impl<M: ManagedTypeApi> UnlockScheduleEx<M> {
    ...
    pub fn reallocate_percentages(&mut self) {
        ...
        let reallocated_milestones = ManagedVec::new();
        let mut new_total = 0;
        for milestone in &self.unlock_milestones {
            ...
            if new_unlock_percentage > 0 {
                ...
                reallocated_milestones.clone().push(UnlockMilestoneEx {
        ...
        if leftover_percent > 0 {
            ...
            let last_milestone = reallocated_milestones.get(last_milestone_index);
            last_milestone.clone().unlock_percent += leftover_percent;

            let _ = reallocated_milestones
                .clone()
                .set(last_milestone_index, last_milestone.clone());
        }
